...
function distributionCount(star: number): number {
  ...
  const half = star < 5 ? (props.distribution[`${star}.5`] ?? 0) : 0
  return full + half
}

const totalCount = computed(() =>
  [1, 2, 3, 4, 5].reduce((sum, s) => sum + distributionCount(s), 0),
)
...
function handleStarClick(star: number) {
  if (!props.clickable)
    return
  emit('update:selectedStar', props.selectedStar === star ? null : star)
}
</script>
  ...
  <v-row align="center">
    ...
    >
      <div
        ...
        :key="star"
        class="d-flex align-center mb-2 gap-2 rounded pa-1"
        :class="{
          'bg-primary-lighten-4': clickable && selectedStar === star,
